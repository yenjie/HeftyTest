...
TANG_WLC_FIG2_HOT_MD_PRIOR = 0.08
TANG_WLC_FIG2_HOT_MS_PRIOR = 0.02
TANG_WLC_FIG2_HOT_CB_PRIOR = 0.18
TANG_WLC_FIG2_HOT_RE_SIGMA_OFFSET_PRIOR = 0.01
TANG_WLC_FIG2_HOT_RE_SIGMA_SCALE_PRIOR = 0.01
TANG_WLC_FIG2_HOT_IM_SIGMA_SCALE_PRIOR = 1.40
TANG_WLC_FIG2_HOT_RE_SIGMA_SLOPE_PRIOR = 0.03
TANG_WLC_FIG2_HOT_IM_SIGMA_SLOPE_PRIOR = 1.50
TANG_WLC_FIG2_HOT_IM_SIGMA_BIAS_PRIOR = 0.20
TANG_WLC_FIG2_HOT_IM_SIGMA_RADIUS_REFERENCE = 0.50
TANG_WLC_FIG2_HOT_IM_SIGMA_RADIUS_PRIOR = 0.35
TANG_WLC_FIG2_HOT_IM_SIGMA_CURVATURE_REFERENCE = -0.20
TANG_WLC_FIG2_HOT_IM_SIGMA_CURVATURE_PRIOR = 0.60
TANG_WLC_FIG2_HOT_PUBLIC_C1_WEIGHT = 0.25
TANG_WLC_FIG2_HOT_SPECTRAL_SHAPE_WEIGHT = 8.0
TANG_WLC_FIG2_HOT_SPECTRAL_SUMMARY_WEIGHT = 50.0
TANG_WLC_FIG2_HOT_CENTROID_WEIGHT = 3.0
TANG_WLC_FIG2_HOT_TARGET_CENTROID_WEIGHT = 18.0
TANG_WLC_FIG2_HOT_TARGET_CENTROID_SIGMA_GEV = 0.12
TANG_WLC_FIG2_HOT_SHOULDER_AMP_REFERENCE = 0.25
TANG_WLC_FIG2_HOT_SHOULDER_AMP_PRIOR = 0.12
TANG_WLC_FIG2_HOT_SHOULDER_OFFSET_REFERENCE = -0.08
TANG_WLC_FIG2_HOT_SHOULDER_OFFSET_PRIOR = 0.06
TANG_WLC_FIG2_HOT_SHOULDER_WIDTH_REFERENCE = 0.50
TANG_WLC_FIG2_HOT_SHOULDER_WIDTH_PRIOR = 0.18
TANG_WLC_FIG2_HOT_SOFT_MODE_AMP_REFERENCE = 0.28
TANG_WLC_FIG2_HOT_SOFT_MODE_AMP_PRIOR = 0.10
TANG_WLC_FIG2_HOT_SOFT_MODE_DROP_REFERENCE = 0.18
...
TANG_WLC_FIG2_MID_RE_SIGMA_OFFSET_PRIOR = 0.05
TANG_WLC_FIG2_MID_RE_SIGMA_SLOPE_PRIOR = 0.20
TANG_WLC_FIG2_MID_IM_SIGMA_SCALE_PRIOR = 0.30
TANG_WLC_FIG2_MID_IM_SIGMA_SLOPE_PRIOR = 0.20
...
# interference function fixed.
TANG_INFERRED_MEDIUM_CORRECTIONS_BY_TEMPERATURE = {
    0.195: {
        "re_sigma_offset": 0.007057180594351814,
        "re_sigma_slope": 0.4862170770075601,
        "im_sigma_scale": 0.8424805669627918,
        "im_sigma_slope": -0.3412034594937484,
    },
    0.251: {
        "re_sigma_offset": 0.017410683917933795,
        "re_sigma_slope": 0.32204357372591186,
        "im_sigma_scale": 1.1355640551727062,
        "im_sigma_slope": -0.41881098432737446,
    },
    0.293: {
        "re_sigma_offset": 0.07161250512004064,
        "re_sigma_slope": 0.3468263825590471,
        "im_sigma_scale": 1.5301552860135021,
        "im_sigma_slope": -0.5246819008023751,
    },
    0.352: {
        "re_sigma_offset": 0.06186841297606755,
        "re_sigma_slope": 0.5607154812541777,
        "im_sigma_scale": 1.9246681007538573,
        "im_sigma_slope": -0.5296805264569785,
    },
}
...
def _tang_inferred_medium_fit(
    *,
    ...
    base = _tang_exact_reference_fit(
        temperature_gev=temperature_gev,
        md=md,
        ms=ms,
        cb=cb,
    ...
    corrections = TANG_INFERRED_MEDIUM_CORRECTIONS_BY_TEMPERATURE[round(float(temperature_gev), 3)]
    return PotentialFit(**{**base.__dict__, **corrections})
...
def _model_spectral_curve(
    *,
    ...
    model_rho = static_spectral_function(
        ...
        spectral_shoulder_amp=fit.spectral_shoulder_amp,
        spectral_shoulder_offset=fit.spectral_shoulder_offset,
        spectral_shoulder_width=fit.spectral_shoulder_width,
...
def _spectral_target_centroid_residuals(
    *,
    temperature_gev: float,
    fit: PotentialFit,
    kernel: SelfEnergyKernel | None,
    ...
    out = []
    weight = float(np.sqrt(total_weight / len(DISTANCES_FM)))
    for distance_fm in DISTANCES_FM:
        _, model_rho = _model_spectral_curve(
            temperature_gev=temperature_gev,
            fit=fit,
        ...
        model_centroid = _spectral_centroid(energy_grid, model_rho)
        target_centroid = _spectral_centroid(target_energies, target_rho)
        out.append(weight * (model_centroid - target_centroid) / TANG_WLC_FIG2_HOT_TARGET_CENTROID_SIGMA_GEV)
    return np.asarray(out, dtype=float)


def _normalize_residual_block(residuals: np.ndarray | list[float], total_weight: float) -> np.ndarray:
    block = np.asarray(residuals, dtype=float)
    if block.size == 0:
        return block
    weight = max(float(total_weight), 0.0)
    if weight == 0.0:
        return np.zeros_like(block)
    return block * np.sqrt(weight / block.size)
...
def _fit_summary(
    ...
    *,
    ...
    return PotentialFit(
        ...
        spectral_shoulder_amp=float(spectral_shoulder_amp),
        spectral_shoulder_offset=float(spectral_shoulder_offset),
        spectral_shoulder_width=float(spectral_shoulder_width),
...
def _resummarize_fixed_forward_fits(
    ...
    for temperature_gev in TEMPERATURES_GEV:
        ...
        residuals = predicted_arr - observed_arr
        summarized[temperature_gev] = PotentialFit(
            **{
                **fit.__dict__,
                "chi2": float(np.sum((residuals / sigma_arr) ** 2)),
                "n_points": int(observed_arr.size),
                "residuals": tuple(float(x) for x in residuals),
                "residual_sigma": tuple(float(x) for x in sigma_arr),
            }
        )
    return summarized
...
def fit_temperature_separately_tang_wlc_forward(
    curves: dict[float, dict[float, LatticeCurve]],
    kernels: dict[float, SelfEnergyKernel],
    phi_values: dict[float, dict[float, float]],
    initial_fits: dict[float, PotentialFit],
) -> dict[float, PotentialFit]:
    lower = np.array([0.2, 0.05, 1.0], dtype=float)
    upper = np.array([1.2, 0.35, 2.5], dtype=float)
    fits: dict[float, PotentialFit] = {}

    for temperature_gev in TEMPERATURES_GEV:
        start = initial_fits[temperature_gev]
        x0 = np.array([start.md, start.ms, start.cb], dtype=float)
        x0 = np.clip(x0, lower, upper)
        ...
        result = least_squares(residuals, x0=x0, bounds=(lower, upper), max_nfev=160)
        fits[temperature_gev] = _tang_exact_reference_fit(
            temperature_gev=temperature_gev,
            md=float(result.x[0]),
            ms=float(result.x[1]),
    ...
    for temperature_gev in TEMPERATURES_GEV:
        ...
        if temperature_gev == 0.352:
            ...
            seed_points = [
                ...
                np.array(
                    [
                        min(publication_parameter_targets[temperature_gev][0], 1.16),
                        publication_parameter_targets[temperature_gev][1],
                        min(publication_parameter_targets[temperature_gev][2], 2.28),
                        min(max(hot_reference["re_sigma_offset"], -0.01), 0.03),
                        hot_reference["im_sigma_scale"],
                        hot_reference["im_sigma_slope"],
                        TANG_WLC_FIG2_HOT_IM_SIGMA_RADIUS_REFERENCE,
                        TANG_WLC_FIG2_HOT_IM_SIGMA_CURVATURE_REFERENCE,
                        TANG_WLC_FIG2_HOT_SOFT_MODE_AMP_REFERENCE,
                        TANG_WLC_FIG2_HOT_SOFT_MODE_DROP_REFERENCE,
                        TANG_WLC_FIG2_HOT_SOFT_MODE_WIDTH_REFERENCE,
                    ],
            ...
            def candidate_from_params(params: np.ndarray) -> PotentialFit:
                ...
                return replace(
                    ...
                    re_sigma_slope=0.0,
                    im_sigma_slope=float(params[5]),
                    im_sigma_bias=0.0,
                    im_sigma_radius=float(params[6]),
                    im_sigma_curvature=float(params[7]),
                    spectral_shoulder_amp=0.0,
                    spectral_shoulder_offset=0.0,
                    spectral_shoulder_width=0.0,
            ...
            mid_lower = np.array([0.15, 0.02, 0.2, -0.08, -0.20, 0.70, -1.20], dtype=float)
            mid_upper = np.array([1.5, 0.7, 3.0, 0.18, 0.90, 2.20, 0.15], dtype=float)
            seed_points = [
                np.array(
                    [
                        start.md,
                        start.ms,
                        start.cb,
                        reference["re_sigma_offset"],
                        reference["re_sigma_slope"],
                        reference["im_sigma_scale"],
                        reference["im_sigma_slope"],
                    ],
                    dtype=float,
                ),
                np.array([start.md, start.ms, start.cb, 0.0, 0.0, 1.0, 0.0], dtype=float),
                np.array(
                    [
                        publication_parameter_targets[temperature_gev][0],
                        publication_parameter_targets[temperature_gev][1],
                        publication_parameter_targets[temperature_gev][2],
                        reference["re_sigma_offset"],
                        reference["re_sigma_slope"],
                        reference["im_sigma_scale"],
                        reference["im_sigma_slope"],
            ...
            def candidate_from_params(params: np.ndarray) -> PotentialFit:
                ...
                return replace(
                    base_fit,
                    re_sigma_offset=float(params[3]),
                    re_sigma_slope=float(params[4]),
                    im_sigma_scale=float(params[5]),
                    im_sigma_slope=float(params[6]),
                )

            bounds = (mid_lower, mid_upper)
        else:
            seed_points = [
        ...
        def residuals(params: np.ndarray) -> np.ndarray:
            ...
            out: list[float] = []
            hot_tang_block: list[float] = []
            hot_public_block: list[float] = []
            hot_c1_block: list[float] = []
            for idx, distance_fm in enumerate(DISTANCES_FM):
                ...
                target = tang[:, 2 + 3 * idx]
                if temperature_gev == 0.352:
                    hot_tang_block.extend(((model - target) / TANG_WLC_FIG2_HOT_TANG_SIGMA_GEV).tolist())
                    ...
                    hot_public_block.extend(((validation_model - curve.m1) / curve.sigma).tolist())
                    c1_target = float(np.interp(distance_fm, public_profile.radius_fm, public_profile.vtilde_gev))
                    c1_sigma = max(
                        float(np.interp(distance_fm, public_profile.radius_fm, public_profile.sigma_gev)),
                        1.0e-6,
                    )
                    hot_c1_block.append(
                        (
                    ...
                    out.extend(
                        (
                            ((validation_model - curve.m1) / curve.sigma)
                            * TANG_WLC_FIG2_PUBLIC_LATTICE_WEIGHT
                        ).tolist()
                    )
                    c1_target = float(np.interp(distance_fm, public_profile.radius_fm, public_profile.vtilde_gev))
                    c1_sigma = max(
                    ...
                    out.append(
                        ...
                        * TANG_WLC_FIG2_PUBLIC_C1_WEIGHT
                    )
                    if idx == len(DISTANCES_FM) - 1:
                        out.extend(
                            [
                                (candidate.md - publication_parameter_targets[temperature_gev][0])
                                / TANG_WLC_FIG2_MD_PRIOR,
                                (candidate.ms - publication_parameter_targets[temperature_gev][1])
                                / TANG_WLC_FIG2_MS_PRIOR,
                                (candidate.cb - publication_parameter_targets[temperature_gev][2])
                                / TANG_WLC_FIG2_CB_PRIOR,
                                (candidate.re_sigma_offset - reference["re_sigma_offset"])
                                / TANG_WLC_FIG2_MID_RE_SIGMA_OFFSET_PRIOR,
                                (candidate.re_sigma_slope - reference["re_sigma_slope"])
                                / TANG_WLC_FIG2_MID_RE_SIGMA_SLOPE_PRIOR,
                                (candidate.im_sigma_scale - reference["im_sigma_scale"])
                                / TANG_WLC_FIG2_MID_IM_SIGMA_SCALE_PRIOR,
                                (candidate.im_sigma_slope - reference["im_sigma_slope"])
            ...
            if temperature_gev == 0.352:
                out.extend(
                    _normalize_residual_block(hot_tang_block, TANG_WLC_FIG2_HOT_TANG_BLOCK_WEIGHT).tolist()
                )
                out.extend(
                    _normalize_residual_block(
                        hot_public_block,
                        TANG_WLC_FIG2_HOT_PUBLIC_LATTICE_WEIGHT,
                    ).tolist()
                )
                out.extend(
                    _normalize_residual_block(
                ...
                out.extend(
                    [
                        (candidate.md - publication_parameter_targets[temperature_gev][0])
                        / TANG_WLC_FIG2_HOT_MD_PRIOR,
                        (candidate.ms - publication_parameter_targets[temperature_gev][1])
                        / TANG_WLC_FIG2_HOT_MS_PRIOR,
                        (candidate.cb - publication_parameter_targets[temperature_gev][2])
                        / TANG_WLC_FIG2_HOT_CB_PRIOR,
                        (candidate.re_sigma_offset - hot_reference["re_sigma_offset"])
                        / TANG_WLC_FIG2_HOT_RE_SIGMA_OFFSET_PRIOR,
                        (candidate.im_sigma_scale - hot_reference["im_sigma_scale"])
                        / TANG_WLC_FIG2_HOT_IM_SIGMA_SCALE_PRIOR,
                        (candidate.im_sigma_slope - hot_reference["im_sigma_slope"])
                        / TANG_WLC_FIG2_HOT_IM_SIGMA_SLOPE_PRIOR,
                        (candidate.im_sigma_radius - TANG_WLC_FIG2_HOT_IM_SIGMA_RADIUS_REFERENCE)
                        / TANG_WLC_FIG2_HOT_IM_SIGMA_RADIUS_PRIOR,
                        (candidate.im_sigma_curvature - TANG_WLC_FIG2_HOT_IM_SIGMA_CURVATURE_REFERENCE)
                        / TANG_WLC_FIG2_HOT_IM_SIGMA_CURVATURE_PRIOR,
                        (candidate.spectral_soft_mode_amp - TANG_WLC_FIG2_HOT_SOFT_MODE_AMP_REFERENCE)
                        / TANG_WLC_FIG2_HOT_SOFT_MODE_AMP_PRIOR,
                        (candidate.spectral_soft_mode_drop - TANG_WLC_FIG2_HOT_SOFT_MODE_DROP_REFERENCE)
                ...
                out.extend(
                    _spectral_shape_residuals(
                        ...
                        fit=candidate,
                        kernel=kernels[temperature_gev],
                        phi_values=phi_values[temperature_gev],
                        spectral_targets=spectral_targets[temperature_gev],
                        total_weight=TANG_WLC_FIG2_HOT_SPECTRAL_SHAPE_WEIGHT,
                    ).tolist()
                )
                out.extend(
                    _spectral_summary_residuals(
                        ...
                        phi_values=phi_values[temperature_gev],
                        spectral_targets=spectral_targets[temperature_gev],
                        total_weight=TANG_WLC_FIG2_HOT_SPECTRAL_SUMMARY_WEIGHT,
                    ).tolist()
                )
                centroid_residuals = _spectral_centroid_residuals(
                    temperature_gev=temperature_gev,
                    fit=candidate,
                    kernel=kernels[temperature_gev],
                    phi_values=phi_values[temperature_gev],
                ...
                out.extend(
                    (
                        centroid_residuals
                        * np.sqrt(TANG_WLC_FIG2_HOT_CENTROID_WEIGHT / CENTROID_ANCHOR_WEIGHT)
                    ).tolist()
                )
                out.extend(
                    _spectral_target_centroid_residuals(
                        temperature_gev=temperature_gev,
                        fit=candidate,
                        kernel=kernels[temperature_gev],
                        phi_values=phi_values[temperature_gev],
                        spectral_targets=spectral_targets[temperature_gev],
                        total_weight=TANG_WLC_FIG2_HOT_TARGET_CENTROID_WEIGHT,
                    ).tolist()
                )
            return np.asarray(out, dtype=float)
...
def summarize_tang_fig2_metrics(
    curves: dict[float, dict[float, LatticeCurve]],
    fits: dict[float, PotentialFit],
    tang_fig2: dict[float, np.ndarray],
    *,
    ...
    per_temperature: dict[str, float] = {}
    for temperature_gev in TEMPERATURES_GEV:
        tang = tang_fig2[temperature_gev]
        tang_tau = tang[:, 0]
        temperature_abs: list[float] = []
        fit = fits[temperature_gev]
        for idx, distance_fm in enumerate(DISTANCES_FM):
            curve = curves[temperature_gev][distance_fm]
            model = _forward_model_curve(
            ...
            target = tang[:, 2 + 3 * idx]
            delta = np.abs(model - target)
            temperature_abs.extend(delta.tolist())
            abs_residuals.extend(delta.tolist())
        per_temperature[f"fig2_mae_T{temperature_gev:.3f}"] = float(np.mean(temperature_abs))
    return {
        "fig2_mae": float(np.mean(abs_residuals)),
        "fig2_max_abs": float(np.max(abs_residuals)),
        **per_temperature,
    }
...
def write_tang_exact_report(
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- Total Euclidean `chi2`: {fit_metrics['chi2']:.6f} for {total_curve_points} points (`chi2/ndof = {fit_metrics['chi2']/n_dof:.6f}` with `ndof = {n_dof}`).",
...
def write_tang_inferred_medium_report(
    output_path: Path,
    curves: dict[float, dict[float, LatticeCurve]],
    intercepts: dict[float, dict[float, InterceptEstimate]],
    global_fits: dict[float, PotentialFit],
    ...
    raw_radius_fm = PUBLIC_WILSON_VALIDATION_RADIUS_INDEX * BAZAVOV_A_FM
    lines = [
        "# Task 1 Tang Replay with Inferred Missing-Medium Corrections Report",
        "",
        "## Scope",
        "",
        "- This branch keeps the Tang Fig. 4 screened-Cornell parameters and the Tang Fig. 3 interference function fixed.",
        "- It also keeps the regularized Tang Fig. 6 reference kernel as the baseline medium input, but adds a low-dimensional temperature-local correction in `(re_sigma_offset, re_sigma_slope, im_sigma_scale, im_sigma_slope)` inferred from the Tang Fig. 2 dashed Euclidean curves.",
        "- The purpose of this branch is diagnostic: identify the smallest effective missing-medium input that improves the Euclidean replay without changing the static screened-Cornell sector.",
        "",
        "## Fixed inputs",
        "",
        f"- Screened-Cornell constants: `alpha_s = {ALPHA_S}` and `sigma = {SIGMA} GeV^2`.",
        "- The screened-Cornell parameters `(m_d(T), c_b(T))` and the common `m_s = 0.2 GeV` are fixed to Tang Fig. 4.",
        "- `phi(r,T)` is fixed to the Tang Fig. 3 interpolators at each temperature.",
        f"- The reference self-energy kernel is inferred from Tang Fig. 6 with Savitzky-Golay smoothing windows `{_format_tang_exact_smoothing()}` for the real part and `{_format_tang_exact_imag_smoothing()}` for the imaginary part on a dense grid of `{TANG_EXACT_REFERENCE_KERNEL_DENSE_ENERGY_POINTS}` points.",
        "",
        "## Summary",
        "",
        f"- Total Euclidean `chi2`: {fit_metrics['chi2']:.6f} for {total_curve_points} points (`chi2/ndof = {fit_metrics['chi2']/total_curve_points:.6f}` with `ndof = {total_curve_points}`).",
        ...
        f"- Tang Fig. 6 mean width mismatch: {fit_metrics['fig6_width_mean']:.6f} GeV.",
        "",
        "## Effective missing-medium corrections",
        "",
    ]
    for temperature_gev in TEMPERATURES_GEV:
        fit = global_fits[temperature_gev]
        lines.append(
            f"- T = {temperature_gev:.3f} GeV: "
            f"re_sigma_offset = {fit.re_sigma_offset:+.6f}, "
            ...
            f"Fig. 2 MAE = {fig2_metrics[f'fig2_mae_T{temperature_gev:.3f}']:.6f} GeV"
        )
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            "## Validation-only checks",
            "",
            "- The public finite-temperature `c1(r,T)` profiles are no longer pure validation in this branch; they enter only as weak intercept anchors, and the values below remain useful to inspect the size of those anchors.",
        ]
    )
    for temperature_gev in TEMPERATURES_GEV:
        profile = public_re_profiles[temperature_gev]
    ...
    lines.extend(
        [
            ...
            "## Caveat",
            "",
            "- These inferred corrections should be interpreted as an effective stand-in for the missing self-consistent medium sector identified in the thesis, not as a unique reconstruction of the unpublished internal Tang inputs.",
            "- The branch is therefore useful for diagnosing what kind of medium correction is needed to recover the dashed Euclidean curves, but it is not a replacement for the full thermodynamic / heavy-light self-consistency loop.",
        ]
    )
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def write_tang_wlc_fit_report(
    output_path: Path,
    curves: dict[float, dict[float, LatticeCurve]],
    intercepts: dict[float, dict[float, InterceptEstimate]],
    global_fits: dict[float, PotentialFit],
    ...
    raw_radius_fm = PUBLIC_WILSON_VALIDATION_RADIUS_INDEX * BAZAVOV_A_FM
    lines = [
        "# Task 1 Tang-Style WLC Fit Report",
        "",
        "## Scope",
        "",
        "- This branch tests the fit step described in Tang 2310.18864 as directly as possible inside the public-data sandbox.",
        "- It keeps the Tang Fig. 3 interference function fixed and keeps the Fig. 6-inferred self-energy kernel fixed after regularization.",
        "- It floats only the screened-Cornell parameters `(m_d(T), m_s(T), c_b(T))` against the public Euclidean WLC benchmark.",
        "",
        "## Fixed inputs",
        "",
        f"- Screened-Cornell constants: `alpha_s = {ALPHA_S}` and `sigma = {SIGMA} GeV^2`.",
        ...
        "- No direct Fig. 5 or Fig. 6 residual term is included in this fit. Those are post-fit comparisons.",
        "",
        "## Fit definition",
        "",
        "- Fit data: the public excited-state-subtracted first cumulants `m1(r,tau,T)` for the four benchmark temperatures and the three benchmark radii, restricted to `tauT <= 0.5`.",
        "- Floated parameters: `m_d(T)`, `m_s(T)`, and `c_b(T)` independently at each benchmark temperature.",
        "- Fixed sectors: `phi(r,T)` and the regularized Fig. 6 reference kernel.",
        "",
        "## Summary",
        "",
        f"- Total Euclidean `chi2`: {fit_metrics['chi2']:.6f} for {total_curve_points} points (`chi2/ndof = {fit_metrics['chi2']/total_curve_points:.6f}` with `ndof = {total_curve_points}`).",
    ...
    lines.extend(
        [
            ...
            "",
            "- The exact finite-temperature raw Wilson-line correlators for `Nt = 36, 28, 24, 20` are still not public in this workspace.",
            "- This fit therefore uses the public excited-state-subtracted benchmark cumulant tables rather than the original line-by-line Wilson-line data stream.",
            f"- The raw-W validation plot remains the public `Nt=56` Bazavov set at `r/a = {PUBLIC_WILSON_VALIDATION_RADIUS_INDEX}` (`r = {raw_radius_fm:.3f} fm`).",
            "",
            "## Intercepts",
            "",
    ...
    lines.extend(
        [
            "## Public c1 references",
            "",
            "- The public finite-temperature `c1(r,T)` profiles enter this branch only as weak intercept anchors; they are not the primary fit target.",
        ]
    )
    for temperature_gev in TEMPERATURES_GEV:
        profile = public_re_profiles[temperature_gev]
    ...
    lines.extend(
        [
            ...
            "## Caveat",
            "",
            "- This branch is closer to the WLC-fit language of Tang 2310.18864 than the fixed replay, but it is still not the full Tang thermodynamic T-matrix calculation because the outer equation-of-state / heavy-light self-consistency loop is not re-solved here.",
            "- Any remaining difference from Tang Fig. 4 therefore measures the impact of fitting `(m_d, m_s, c_b)` alone inside the public static sandbox with fixed `phi(r,T)` and a fixed reference kernel.",
        ]
    )
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_tang_wlc_fig2_fit_report(
    ...
    lines = [
        ...
        "## Scope",
        "",
        "- This branch floats `(m_d(T), m_s(T), c_b(T))` while keeping the Tang Fig. 3 interference function fixed and the Fig. 6-inferred self-energy kernel fixed up to a small temperature-dependent medium correction above `T=0.195 GeV`.",
        "- Unlike the public-lattice WLC fit branch, this branch keeps the dashed Euclidean curves of Fig. 2 in Tang 2310.18864 as the primary target.",
        "- The public excited-state-subtracted lattice benchmark and public finite-temperature `c1(r,T)` profiles enter only as weak secondary anchors.",
        "",
        "## Fixed inputs",
        "",
        f"- Screened-Cornell constants: `alpha_s = {ALPHA_S}` and `sigma = {SIGMA} GeV^2`.",
        ...
        "## Summary",
        "",
        f"- Public-lattice post-fit score: {fit_metrics['chi2']:.6f} for {total_curve_points} points (`score/Npts = {fit_metrics['chi2']/total_curve_points:.6f}`).",
        f"- Tang Fig. 2 dashed-curve MAE: {fig2_metrics['fig2_mae']:.6f} GeV; max deviation: {fig2_metrics['fig2_max_abs']:.6f} GeV.",
        f"- Tang Fig. 4 `L1`: {fit_metrics['fig4_l1']:.6f}.",
        f"- Tang Fig. 5 potential MAE: {fit_metrics['fig5_mae']:.6f} GeV.",
        f"- Tang Fig. 6 mean peak mismatch: {fit_metrics['fig6_peak_mean']:.6f} GeV.",
    ...
    for temperature_gev in TEMPERATURES_GEV:
        ...
        if 0.251 <= temperature_gev < 0.352:
            line += (
                f"; medium correction: re_sigma_offset = {fit.re_sigma_offset:.6f} GeV, "
                f"re_sigma_slope = {fit.re_sigma_slope:.6f}, "
                f"im_sigma_scale = {fit.im_sigma_scale:.6f}, "
                f"im_sigma_slope = {fit.im_sigma_slope:.6f}"
            )
    ...
    lines.extend(
        [
            ...
            "",
            "- The exact finite-temperature raw Wilson-line correlators for `Nt = 36, 28, 24, 20` are still not public in this workspace.",
            "- This branch therefore uses the published Fig. 2 curves as the primary Euclidean target and supplements them with weak anchors from the public excited-state-subtracted cumulant tables and the public finite-temperature `c1(r,T)` profiles.",
            f"- The raw-W validation plot remains the public `Nt=56` Bazavov set at `r/a = {PUBLIC_WILSON_VALIDATION_RADIUS_INDEX}` (`r = {raw_radius_fm:.3f} fm`).",
            "",
            "## Intercepts",
            "",
...
def run_task1_tang_wlc_fig2_fit_benchmark(
    *,
    ...
    payload = {
        "metadata": {
            "method": "tang_fig2_wlc_fit_with_floating_screened_cornell_and_temperature_ordered_medium_tradeoff_corrections",
            "tau_half_max": TAU_HALF_MAX,
            "intercept_points": DEFAULT_INTERCEPT_POINTS,
...
def run_task1_tang_wlc_fit_benchmark(
    *,
    root: Path,
    output_dir: Path,
) -> dict[str, object]:
    ...
    global_fits = fit_temperature_separately_tang_wlc_forward(
        curves,
        kernels,
        phi_values,
        initial_fits,
    ...
    plot_m1_reproduction(
        ...
        fixed_kernels=kernels,
        fixed_phi_values=phi_values,
        title="Task 1: Tang-style WLC fit with floating md/ms/cb",
    )
    plot_vtilde_extracted(intercepts, global_fits, global_fits, tang_fig5, vtilde_plot)
    plot_fit_performance(intercepts, global_fits, performance_plot)
    plot_public_zero_temperature_wilson_validation(raw_wilson_validation, raw_wilson_plot)
    ...
    plot_phi_extracted(
        ...
        phi_plot,
        fixed_phi_interpolators=phi_interpolators,
        title="Task 1: fixed Tang interference with floating screened-Cornell fit",
    )
    plot_fig4_parameter_comparison(root, global_fits, fig4_plot)
    plot_spectral_extraction(spectral_outputs, spectral_plot)
    plot_spectral_ansatz_sensitivity(ansatz_sensitivity, spectral_ansatz_plot)
    ...
    payload = {
        "metadata": {
            "method": "tang_wlc_fit_with_floating_screened_cornell_parameters",
            "tau_half_max": TAU_HALF_MAX,
            "intercept_points": DEFAULT_INTERCEPT_POINTS,
            "finite_temperature_lattice_input": "public_subtracted_m1_tables_only",
            "finite_temperature_raw_wilson_publicly_available": False,
            ...
            "self_consistent_closure": "paper-like static WLC fit with fixed phi(r,T), fixed regularized reference kernel, and floating screened-Cornell parameters only; no outer equation-of-state loop is re-solved",
        },
        "fit_metrics": {**fit_metrics, **fig2_metrics},
        "global_common_ms_fit": {
            f"{temperature_gev:.3f}": _fit_record(global_fits[temperature_gev])
            for temperature_gev in TEMPERATURES_GEV
        },
...
def run_task1_tang_inferred_medium_benchmark(
    *,
    root: Path,
    output_dir: Path,
) -> dict[str, object]:
    ...
    separate_fits = {
        temperature_gev: _tang_inferred_medium_fit(
            temperature_gev=temperature_gev,
            md=publication_parameter_targets[temperature_gev][0],
            ms=publication_parameter_targets[temperature_gev][1],
            cb=publication_parameter_targets[temperature_gev][2],
    ...
    plot_m1_reproduction(
        ...
        fixed_kernels=kernels,
        fixed_phi_values=phi_values,
        title="Task 1: Tang replay with inferred missing-medium corrections",
    )
    plot_vtilde_extracted(intercepts, separate_fits, global_fits, tang_fig5, vtilde_plot)
    plot_fit_performance(intercepts, global_fits, performance_plot)
    plot_public_zero_temperature_wilson_validation(raw_wilson_validation, raw_wilson_plot)
    ...
    plot_phi_extracted(
        ...
        phi_plot,
        fixed_phi_interpolators=phi_interpolators,
        title="Task 1: fixed Tang interference function replay with inferred medium corrections",
    )
    plot_fig4_parameter_comparison(root, global_fits, fig4_plot)
    plot_spectral_extraction(spectral_outputs, spectral_plot)
    plot_spectral_ansatz_sensitivity(ansatz_sensitivity, spectral_ansatz_plot)
    spectral_json.write_text(json.dumps(spectral_outputs, indent=2) + "\n", encoding="utf-8")
    spectral_ansatz_json.write_text(json.dumps(ansatz_sensitivity, indent=2) + "\n", encoding="utf-8")
    write_tang_inferred_medium_report(
        report_md,
        curves,
        intercepts,
        global_fits,
    ...
    payload = {
        "metadata": {
            "method": "thesis_guided_fixed_phi_replay_with_inferred_missing_medium_corrections",
            "tau_half_max": TAU_HALF_MAX,
            "intercept_points": DEFAULT_INTERCEPT_POINTS,
            "finite_temperature_lattice_input": "public_subtracted_m1_tables_only",
            "finite_temperature_raw_wilson_publicly_available": False,
            "fixed_phi_source": "Tang 2310.18864 Fig3 interpolators",
            "reference_self_energy_source": f"Tang 2310.18864 Fig6 inferred kernel with effective low-dimensional missing-medium correction (Re: {_format_tang_exact_smoothing()}; Im: {_format_tang_exact_imag_smoothing()}; dense grid: {TANG_EXACT_REFERENCE_KERNEL_DENSE_ENERGY_POINTS})",
            "fixed_ms_gev": 0.2,
            "fit_parameters": [
                "re_sigma_offset(T)",
                "re_sigma_slope(T)",
            ...
            "self_consistent_closure": "exact Tang Fig. 4 screened-Cornell replay with fixed phi(r,T), fixed regularized reference kernel, and an inferred low-dimensional self-energy correction chosen to reproduce the Tang Fig. 2 dashed Euclidean curves as closely as possible while staying near Fig. 6",
        },
        "fit_metrics": {**fit_metrics, **fig2_metrics},
        "separate_fit": {
            f"{temperature_gev:.3f}": _fit_record(separate_fits[temperature_gev])
